...
export default function ProductCard({ item, isWishlist = false }) {
    ...
    return (
        <div className={`bg-slate-200 shadow-2xl dark:bg-slate-600 p-3 rounded-2xl transition-all duration-300
        ${isTouched ? 'scale-105 shadow-xl' : 'hover:scale-105'}`}
            ...
            <div className="relative group rounded-2xl overflow-hidden" onTouchStart={() => setIsTouched((prev) => !prev)} onMouseLeave={() => setIsTouched(false)}>
                ...
                <img src={item.imageCover} alt={item.title} loading="lazy" className={`rounded-2xl w-full object-cover transition-transform duration-500
            ${isTouched ? 'scale-125 filter blur-sm' : 'group-hover:scale-125'}`} />
                {/* Overlay Icons */}
                <div className={`absolute inset-0 bg-white/30 dark:bg-black/30 backdrop-blur-xs flex justify-center items-center gap-4 transition-opacity duration-300 z-10
            ${isTouched ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <div className="flex gap-4">
                        {/* Add to Cart */}
                        {!isWishlist && (
                            <div onClick={() => dispatch(addProductToCart(item._id))} className={`bg-mainColor rounded-full p-3 cursor-pointer text-white transition-all duration-300 delay-75
                  ${isTouched ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5 group-hover:opacity-100 group-hover:translate-y-0'}`}                            >
                                <ShoppingCart size={30} className="transition-transform duration-300 ease-in-out hover:scale-125 hover:rotate-12" />
                            </div>
                        )}
                        {/* Heart Icon */}
                        <div onClick={handleWishlistAction} className={`bg-mainColor rounded-full p-3 cursor-pointer transition-all duration-300 delay-150
                ${isTouched ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5 group-hover:opacity-100 group-hover:translate-y-0'}`}>
                        ...
                        {/* Eye Icon */}
                        <Link to={`/products/${item._id}`} className={`bg-mainColor rounded-full p-3 cursor-pointer text-white transition-all duration-300 delay-200
                ${isTouched ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5 group-hover:opacity-100 group-hover:translate-y-0'}`}>
